...
1. ~~CropRotateFilter C++ 实现~~ → fragment shader UV 逆变换（90°旋转 + 自由旋转 + 裁剪映射），作为 FilterChain 首个滤镜，输出 FBO 尺寸 = 裁剪后像素尺寸
2. ~~裁剪 UI~~ → CropOverlay Compose Canvas，四角 + 四边拖拽手柄，暗化遮罩 + 九宫格网格线，支持 自由/1:1/4:3/16:9 比例约束
3. ~~旋转 UI~~ → 90° 快捷旋转按钮 + 自由旋转滑条 (-45°~+45°)，CropToolBar 底部工具栏
4. ~~独立裁剪模式~~ → EditorScreen 切换裁剪/滤镜模式，裁剪时 GL 渲染不含 crop（仅旋转+滤镜），Compose 叠加 CropOverlay
5. ~~导出适配~~ → 按裁剪后尺寸导出，C++ exportImage 使用输出纹理实际尺寸
6. **测试点**：裁剪 + 旋转 + 滤镜叠加后，预览与导出一致 ✅

### Phase 11 — 撤销/重做 ✅

参数快照栈，支持操作回退和重做。

1. ~~参数快照数据类~~ → `EditorSnapshot` data class，快照所有滤镜参数 + LUT 预设 + 裁剪/旋转状态
2. ~~撤销/重做栈~~ → 纯 Kotlin 双栈实现，滑条拖拽整段为一个 undo 单元（beginParameterChange + commitParameterChange），离散操作（选预设、旋转、比例切换）自动压栈
3. ~~裁剪模式~~ → 整个裁剪会话为一个 undo 单元（进入时存快照，确认时压栈，取消时恢复）
4. ~~UI~~ → 图片预览区左下角浮动透明箭头按钮（↩ / ↪），有操作可撤销/重做时才显示
5. **测试点**：多次调参后撤销/重做，参数和预览正确恢复 ✅
